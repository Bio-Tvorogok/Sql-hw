...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def get_response(self) -> dict:
        ...
        # Реализуем API /rates/user_id
            ...
            response = self.get_user_watch_history(user_id)
    ...
    def get_user_watch_history(self, user_id) -> dict:
        """
        ...
        if redis_interactor.is_cached(redis_profile_key):
            logging.info(f'Оценки пользователя user_id={user_id} присутствуют в кеше')
            response = redis_interactor.get_data(redis_profile_key)
        else:
            try:
                logging.info(f'Выполняем запрос к БД, чтобы получить оценки пользователя user_id={user_id}')
            ...
            response = {
                key: {
                    ...
                    'rating': value[1],
                    'timestamp': value[2]
                } for key, value in enumerate(user_rates)
            }
